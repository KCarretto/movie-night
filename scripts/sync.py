#!/usr/bin/env python3
"""Build-time sync for ``movies.pbf``, the Movie Night recommendation DB.
...
with this script and commit the resulting ``movies.pbf`` so the runtime app
stays key-free and serverless. ``movies.pbf`` is a dense Protobuf binary
described by ``scripts/catalog.proto`` (see ``scripts/catalog_io.py`` for the
dict <-> message translation); it replaces the older plain-text ``movies.json``
to cut transfer size and parse cost in the browser.
...
def poster_url(poster_path: Optional[str]) -> str:
    """Return the raw relative TMDB poster path (empty string when none is set).

    The verbose base domain (``TMDB_IMAGE_BASE``) is intentionally *not* stored:
    only the relative suffix (e.g. ``/lh4v5.jpg``) is serialized to keep the
    binary catalogue lean, and the browser re-prefixes the base at load time.
    """
    if not poster_path:
        return ""
...
def load_existing(path: str) -> List[Dict[str, Any]]:
    ...
    Reads the dense Protobuf binary (``movies.pbf``) and inflates each record
    back to the internal dict representation used throughout the sync.
    """
    try:
        movies = catalog_io.load_catalog(path)
    except Exception:
        return []
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Sync movies.pbf for Movie Night.")
    parser.add_argument(
        "--pages",
        type=int,
        default=25,
    ...
    parser.add_argument(
        "--output",
        default=os.path.join(os.path.dirname(os.path.dirname(__file__)), "movies.pbf"),
        help="Path to write the Protobuf catalogue. Default: <repo>/movies.pbf.",
    )
    parser.add_argument(
        "--ids-file",
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    # Existing movies are appended first (so resume order is preserved), then
    # each newly downloaded movie is converted and appended as it streams in.
    catalog = catalog_io.catalog_pb2.MovieCatalog()
    total = 0
    try:
        for movie in existing:
            catalog_io.movie_dict_to_proto(movie, catalog.movies.add())
            total += 1
        del existing
        ...
        catalog_io.save_catalog_proto(args.output, catalog)
    except Exception:
        raise
